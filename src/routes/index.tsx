...
function Hero() {
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-10 pt-12 pb-20 lg:pt-24 lg:pb-32 grid lg:grid-cols-12 gap-10 items-center">
        <motion.div
        ...
        >
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-background/70 backdrop-blur border border-border text-xs uppercase tracking-[0.18em]">
            <Sparkles className="h-3 w-3 text-primary" /> Spring 2026 — In bloom
          </span>
          <h1 className="mt-6 font-serif text-5xl md:text-6xl lg:text-7xl leading-[1.05] text-balance">
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            ...
            <Link to="/tutorials" className="text-sm underline-offset-4 hover:underline">
              Watch tutorials →
            </Link>
          </div>
        ...
        >
          <div className="relative aspect-[4/5] rounded-[2rem] overflow-hidden shadow-petal">
            <img
              src={heroModel}
          ...
          <motion.div
            ...
            className="absolute -bottom-6 -left-4 md:-left-10 bg-card/90 backdrop-blur border border-border rounded-2xl px-5 py-4 shadow-petal max-w-[220px]"
          >
            <p className="text-xs text-muted-foreground">Bestseller</p>
            <p className="font-serif text-lg leading-tight mt-1">Velvet Bloom in Rosé</p>
            <p className="text-xs mt-1">$32 · 12 shades</p>
          </motion.div>
        </motion.div>
      </div>
    </section>
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 lg:px-10 py-24">
      <div className="flex items-end justify-between mb-12">
        <div>
          <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground">Trending looks</p>
          <h2 className="mt-3 font-serif text-4xl md:text-5xl">In bloom this week</h2>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        <motion.div
          ...
          className="lg:col-span-7 group relative aspect-[4/3] overflow-hidden rounded-3xl"
        >
          ...
          <div className="absolute bottom-6 left-6 right-6 text-background">
            <p className="text-xs uppercase tracking-[0.25em] opacity-80">{looks[0].label}</p>
            <h3 className="mt-2 font-serif text-3xl md:text-4xl">{looks[0].title}</h3>
          </div>
        </motion.div>
          ...
          {looks.slice(1).map((l, i) => (
            ...
            >
              ...
              <div className="absolute bottom-5 left-5 right-5 text-background">
                <p className="text-[10px] uppercase tracking-[0.25em] opacity-80">{l.label}</p>
                <h3 className="mt-1 font-serif text-2xl">{l.title}</h3>
              </div>
            </motion.div>
          ))}
        </div>
    ...
    <section className="mx-auto max-w-7xl px-6 lg:px-10 py-12">
      <div className="flex items-end justify-between mb-12">
        <div>
          ...
          <h2 className="mt-3 font-serif text-4xl md:text-5xl">Featured petals</h2>
        </div>
...
function Testimonials() {
  ...
  return (
    <section className="bg-blossom/60 py-24 mt-12">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground text-center">Whispered, then loved</p>
        <h2 className="mt-3 font-serif text-4xl md:text-5xl text-center">Petal stories</h2>
        <div className="mt-14 grid gap-6 md:grid-cols-3">
          {reviews.map((r, i) => (
            ...
            >
              ...
              <blockquote className="font-serif text-xl leading-snug">"{r.quote}"</blockquote>
